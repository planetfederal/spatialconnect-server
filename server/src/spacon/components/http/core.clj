...
(defrecord HttpService [http-config ping user team device location store config queue form notify]
  component/Lifecycle
  (start [this]
    ...
    (let [routes #(route/expand-routes
                   (clojure.set/union #{}
                                      ...
                                      (store-http/routes queue store)
                                      (config-http/routes config)
                                      (form-http/routes form queue)
